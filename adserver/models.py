...
class Flight(TimeStampedModel, IndestructibleModel):
    ...
    * Whether the flight is live or not
    ...
    def views_today(self, bypass_cache=False):
        ...
        today = get_ad_day().date()
        cache_key = f"flight_views_today_{self.pk}_{today}"
        ...
        aggregation = AdImpression.objects.filter(
            advertisement__in=self.advertisements.all(), date=today
    ...
    def clicks_today(self, bypass_cache=False):
        ...
        # Otherwise, populate the local cache
        today = get_ad_day().date()
        cache_key = f"flight_clicks_today_{self.pk}_{today}"
        cached_value = caches[settings.CACHE_LOCAL_ALIAS].get(cache_key)
        if cached_value is not None and not bypass_cache:
            return cached_value
    ...
    @classmethod
    def create_invoice(cls, flights):
        """Create a new draft invoice for the given flights."""
        if not settings.STRIPE_ENABLED:
            raise ValueError(
                _("Stripe is not configured. Please set STRIPE_SECRET_KEY.")
            )

        if not flights:
            # No flights to create an invoice for. Don't error, just do nothing.
            return None

        if len({f.campaign.advertiser_id for f in flights}) > 1:
            raise ValueError(
                _(
                    "All flights must must be from a single advertiser to create an invoice."
                )
            )
        ...
        if not advertiser.djstripe_customer:
            raise ValueError(
                _("Advertiser does not have an associated Stripe customer.")
            )
